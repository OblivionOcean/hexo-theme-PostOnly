...
(function() {
  ...
  postNote._generateNoteHtml = function(args) {
    ...
    if (args.length > 1) {
      var cls = args[0].trim();   // 样式类名
      var text = args[1].trim();  // 提示内容
      // 使用hexo渲染引擎将markdown内容转换为HTML
      return '<div class="note ' + cls + '">' + postNote._renderMarkdown(text) + '</div>';
    } 
    // 如果只有一个参数，则只有内容
    else if (args.length > 0) {
      var textOnly = args[0].trim();  // 提示内容
      // 使用hexo渲染引擎将markdown内容转换为HTML
      return '<div class="note">' + postNote._renderMarkdown(textOnly) + '</div>';
    }
    return '';
  };
  ...
  hexo.extend.filter.register('before_post_render', function(data) {
    ...
    data.content = data.content.replace(/{%\s+noteblock(.*)%}/g, function(p,q) {
      return '{% blocknote ' + q + ' %}';
    });
